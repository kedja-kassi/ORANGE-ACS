...
WebUI.setText(findTestObject('Page_Recherche/input_Numro de srie_form-contr'), GlobalVariable.SN_ALCL)
...
WebUI.waitForElementPresent(findTestObject('Page_Recherche/table_Etat'), 5)

String ExpectedValue = GlobalVariable.SN_ALCL
...
'To locate table'
WebElement table = driver.findElement(By.id('searchtable'))
    ...
    Loop: for (int row = 0; row < rows_count; row++) {
        ...
        for (int column = 0; column < columns_count; column++) {
            ...
            if (celltext == ExpectedValue) {
                ...
                println('Le SN recherché est : ' + Columns_row.get(column).getText())

                'After getting the Expected value from Table we will Terminate the loop'
                break
            } else {
                throw new com.kms.katalon.core.exception.StepFailedException('Le SN recherché nest pas celui qui à été trouvé')
            }
        }
    }
}

WebUI.clearText(findTestObject('Page_Recherche/input_Numro de srie_form-contr'))
