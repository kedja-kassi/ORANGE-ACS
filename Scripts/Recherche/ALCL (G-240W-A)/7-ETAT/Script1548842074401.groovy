...
Loop: for (int row = 2; row < rows_count; row++) {
	...
	println('Texte de la colonne 1 retrouvé :' + Columns_row.get(1).getText())
	println('classe de la colonne 1 retrouvé:' + Columns_row.get(1).getAttribute("class"))
	/*String celltext = Columns_row.get(3).getCssValue('class')
	println('classe retrouvé 2:' + celltext)
	println('classe retrouvé 1:' + Columns_row.get(3).getAttribute("class"))*/
